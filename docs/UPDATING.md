...
Если стояла `v26.1`, а последним релизом стала `v26.2`, установщик увидит разницу, явно напишет «Найдена версия 26.1 — будет выполнено обновление до 26.2», сделает резервную копию и поставит новый выпуск. Если вместо этого запросить версию старше уже установленной (`--version`/`-Version`), сообщение так и скажет — «будет выполнен откат», а не «обновление». Повторный запуск на той же версии ничего не перезаписывает и сообщает, что версия уже актуальна; с `--force`/`-Force` установщик прямо предупредит, что переустанавливает ту же версию.
