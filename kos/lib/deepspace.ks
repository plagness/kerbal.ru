...
//   SET s["body"]    TO MUN.
//   SET s["parkAlt"] TO 18000.     // низкая орбита — отсюда выпускаем лендеры
//   SET s["mapAlt"]  TO 250000.    // картографическая полярная — тут сканеры
//   SET s["landers"] TO LIST("lander-1", "lander-2", "lander-3").
//   SET s["sci"]     TO TRUE.      // снимать науку на каждом рубеже
//   ds_run(s).
//
// Тот же принцип, что у m_orbit() в lib/mission.ks: файл миссии содержит
// только параметры цели, порядок шагов и решения по ходу («сколько лендеров»,
// «нужна ли подгонка плоскости», «когда включать сканеры») — здесь, одним
// местом. Наклонение к телу этим модулем НЕ задаётся — закладывай его на
// пуске у родителя, см. [[rendezvous]].
...
GLOBAL FUNCTION ds_defaults {
  RETURN LEXICON(
    "name", "дальний вылет",
    "body", MUN,
    "parkAlt", 18000,     // низкая орбита захвата, м — старт для лендеров
    "mapAlt", 250000,     // картографическая полярная, м — тут работают сканеры
    "landers", LIST(),    // метки декаплеров, пусто = лендеров нет
    "sci", TRUE,          // снимать науку на рубежах (переход/захват/орбита)
    "log", "0:/deepspace.csv"
  ).
}
...
GLOBAL FUNCTION ds_run {
  ...
  IF spec["landers"]:LENGTH > 0 {
    PRINT "=== выпуск лендеров на низкой орбите " + ROUND(spec["parkAlt"]/1000,1) + " км".
    PRINT "  разносим по доле витка, чтобы сесть в разные биомы —".
    PRINT "  точную точку посадки выбирает игрок, автоматом не подгоняется.".
  ...
  IF spec["landers"]:LENGTH > 0 {
    PRINT "  посадка лендера — после переключения фокуса на него:".
