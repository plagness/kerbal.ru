...
GLOBAL FUNCTION l_descend {
  ...
  //
  // НО: этот варп целится по ETA:PERIAPSIS — а перицентр КЕПЛЕРОВОЙ
  // орбиты при крутом падении (перицентр глубоко отрицательный, «под
  // землёй») орбита никогда не достигнет, ETA:PERIAPSIS для реального
  // момента столкновения ничего не значит. Управление (LOCK STEERING/
  // THROTTLE) включается только ПОСЛЕ этого варпа — на крутом заходе
  // варп проскакивал мимо безопасного окна для торможения, и торможение
  // включалось буквально у земли на смертельной скорости (аппарат
  // разбился о Муну на ~500 м/с). Поймано вживую. Поэтому варп — только
  // на пологом заходе (перицентр не глубже -5 км от поверхности); на
  // крутом падении управление включаем немедленно, без варпа вообще.
  IF SHIP:PERIAPSIS > -5000 AND ALT:RADAR > 15000 AND ETA:PERIAPSIS > 30 {
    ...
    PRINT "  ! крутое падение (перицентр " + ROUND(SHIP:PERIAPSIS/1000,1) + " км) — без варпа, торможу немедленно.".
  }

  PRINT "=== посадка: жду касания поверхности".
  SAS OFF.
  c_rcs(TRUE).
  // Горизонтальная скорость относительно поверхности — раньше ниже
  ...
  LOCK THROTTLE TO 0.
  UNLOCK STEERING.
  // По требованию оператора: САС и РСУ включаем СРАЗУ после касания, а не
  // выключаем — стабилизация ориентации сразу после посадки должна гасить
  // остаточные колебания, а не давать им раскачать аппарат без управления.
  SAS ON.
  RCS ON.
