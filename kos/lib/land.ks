...
GLOBAL FUNCTION l_descend {
  ...
  UNLOCK STEERING.
  RCS OFF.
  // НЕ включаем SAS сразу: реактивные колёса резко дёргают в последнюю
  // запертую ориентацию (STEERING TO UP), и на уклоне/неровном грунте
  // этот рывок опрокидывает аппарат ПОСЛЕ уже чистого касания (SHIP:STATUS
  // уже был LANDED). Поймано вживую дважды подряд. Даём аппарату спокойно
  // осесть без активного управления ориентацией.
  WAIT 2.
