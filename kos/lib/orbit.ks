...
GLOBAL FUNCTION o_burn {
  ...
  // Разворот на Солнце перед долгим варпом здесь ОТКАТАН: разворот на
  // Солнце → SAS → сразу жёсткий LOCK STEERING на вектор прожига в
  // момент выхода из варпа дал резкий рывок маховиков и раскрутку
  // корабля (поймано вживую). Наведение на узел точное и чувствительное
  // к моменту — не то место для эксперимента с ориентацией. Экономия
  // питания на этом холостом участке сделана в l_descend, где после
  // варпа нет активного прожига и рывок не так опасен.
