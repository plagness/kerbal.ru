...
//
// НАЙДЕНО И ПОЧИНЕНО (2026-07-31, после третьей живой попытки): причиной
// отрицательного (тормозного) узла был radius, посчитанный через
// POSITIONAT(SHIP, ut) − POSITIONAT(SHIP:BODY, ut) — вживую дал 14 388 020 м
// вместо ожидаемых ~679 000 (наша реальная высота + радиус Кербина).
// Проверено печатью всех промежуточных величин прямо в полёте: tgtSma и
// vNow оказались верными (12 000 000 и 2282 м/с), сломан был только rad —
// POSITIONAT в этой связке считает не то, что нужно (детально причину
// внутри POSITIONAT не разбирали, ушли от неё вовсе). Замена — текущая
// высота вместо экстраполяции: орбита парковочная, почти круговая, за
// горизонт в 1000–1500 с радиус меняется на единицы метров, той же
// логикой уже пользуется u_utAtArgLat в util.ks.
