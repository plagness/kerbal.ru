...
Внутри — та же последовательность, что при рандеву с аппаратом ([[rendezvous]]), только целью выступает небесное тело: подгонка плоскости → окно перехода → прицельный перицентр у чужой сферы влияния (пробой знака, тот же приём, что у `r_nodeMatchPlanes`) → захват в перицентре (`o_nodeCircularizeAtPeri` — у гиперболы апоцентра нет, поэтому не тот узел, что при обычном скруглении). Дальше, если в `spec` заданы `landers`, — выпуск по очереди с интервалом в треть витка, чтобы наземные треки разошлись.

**Что автоматика не решает сама:** какой конкретно биом выбрать под каждый лендер (наземный трек — не гарантия, только шанс попасть в разные) и включение сканеров SCANsat — их модуль в библиотеке не сверен, переключаются с приборной панели руками. Посадка каждого лендера — отдельный шаг после переключения фокуса, `l_run(s)` из `lib/land.ks`: kOS не может вести аппарат, на который не переключен фокус, см. [[flot]].
