...
**Префикс в имени = модуль.** `u_` — util, `o_` — orbit, `a_` — ascent, `s_` — stage, `c_` — ctrl, `d_` — deploy, `t_` — telem, `h_` — hud, `m_` — mission, `r_` — rendezvous, `x_` — transfer, `l_` — land, `e_` — events, `ds_` — deepspace. У науки исторически `sci`. Это не украшение: в kOS все имена глобальны, префикс — единственная защита от столкновений.
