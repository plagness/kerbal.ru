...
| `ds_run(spec)` | лексикон | перелёт → выпуск лендеров → картографическая орбита, весь цикл |

Ключи `spec`: `name`, `body` (тело, по умолчанию `MUN`), `parkAlt` (низкая орбита захвата, 18 км — отсюда выпускают лендеров), `mapAlt` (картографическая полярная, 250 км), `landers` (список меток декаплеров, пусто = лендеров нет), `sci` (снимать науку на рубежах), `log`.

Лендеров `ds_run` разносит по равным долям витка, чтобы у них были разные наземные треки — точный выбор биома под каждым остаётся за игроком, автомат этого не гарантирует. Сканеры SCANsat скрипт не включает: их модуль и имена событий в библиотеке не сверены.
...
И по телу — `q_bodyDefaults()` / `q_bodyChecklist(spec)` / `q_bodyMun()` (готовый пресет: Δv ≥ 1150 м/с, заряд на теневую сторону ≥ 1400 ЭЧ либо РИТЭГ). `spec["dv"]` сверяется со **стоковым** счётчиком дельта-v игры (`SHIP:DELTAV:VACUUM`, KSP 1.11+) — если в настройках отключён Advanced Tweakables → Delta-V readout, вернётся 0, и это будет читаться как «не хватает», хотя на деле просто выключен показ.

**Чего чекер не видит** — и не должен: хватит ли топлива именно этому лендеру, мягко ли он сядет, попадёт ли в нужный биом. Каждый готовый прогон (`check/dalnyak-mun-audit.ks`) печатает список таких слепых зон отдельной строкой — не молчит о том, что не проверено.
