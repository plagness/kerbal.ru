...
Спецификацию под конкретный корабль в файл не сохраняют — собирают `tasks`/`budget` из готовых `q_task*`/`q_body*` и шлют `q_plan(spec)` прямо в терминал (через telnet — одной строкой, см. [[kos-telnet]]). Ни двигателей, ни узлов манёвра — только чтение и арифметика, поэтому это безопасно гонять хоть каждый раз заново.
